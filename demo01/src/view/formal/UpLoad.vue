  ...
  <div class="demo-progress">
    <div>
      ...
      <el-progress
          ...
          show-text
      >
        <el-text truncated class="font-style">数据分类({{ this.process_value_3.value }}%)</el-text>
      </el-progress>
...
export default {
  name: "UpLoad",
  props: ["accept_suffix", "max_file_size", "limit_file_number"],
  emits: ["loadOver", "loadReset"],
  data() {
    return {
      uploadRef: ref<UploadInstance>(), // 上传行为
      fileList: ref<UploadUserFile[]>([]), // 上传队列
      process_value_1: reactive({value: 0}), // 第一个进度条进度 ts ref在进行参数传递时会以number形式传入，需要使用reactive
      process_value_2: reactive({value: 0}), // 第二个进度条进度
      process_value_3: reactive({value: 0}), // 第三个进度条进度
      page_judge: true,
  ...
  methods: {
    ...
    handleExceed(files, uploadFiles) {
      ElMessage.warning(
          `The limit is ${this.limit_file_number}, you selected ${files.length} files this time, add up to ${
    ...
    processComplete() {
      this.$emit("loadOver", null)
    },
    processReset() {
      this.$emit("loadReset", null)
    },
    sleep(ms) {
